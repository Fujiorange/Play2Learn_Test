...
// Get level-based quizzes (1-10) - Auto-generated quizzes for students
router.get('/level-quizzes', authenticateToken, async (req, res) => {
  try {
    // Fetch all auto-generated quizzes with quiz_level set (1-10)
    // These are the new level-based quizzes, not teacher-launched adaptive quizzes
    const quizzes = await Quiz.find({
      is_active: true,
      is_auto_generated: true,
      quiz_level: { $exists: true, $ne: null }
    })
    .select('title description quiz_level questions createdAt generation_criteria')
    .sort({ quiz_level: 1 }) // Sort by level ascending
    .lean();

    // Group quizzes by level and count questions
    const levelQuizzes = quizzes.map(quiz => {
      const difficultyCount = {};
      quiz.questions.forEach(q => {
        const diff = q.difficulty || 3;
        difficultyCount[diff] = (difficultyCount[diff] || 0) + 1;
      ...
      return {
        ...
        total_questions: quiz.questions.length,
        difficulty_distribution: difficultyCount,
        generation_criteria: quiz.generation_criteria,
        createdAt: quiz.createdAt
      };
    });
    ...
    res.status(500).json({
      success: false,
      error: 'Failed to fetch level quizzes'
    });
  }
});
