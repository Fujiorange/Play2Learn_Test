// TakeQuiz.js - Regular Quiz (After Placement)
...
export default function TakeQuiz() {
  ...
  useEffect(() => {
    const loadQuiz = async () => {
      ...
      try {
        ...
        const result = await studentService.generateQuiz();
  ...
  const styles = {
    ...
    profileBadge: { padding: '8px 16px', background: 'linear-gradient(135deg, #3b82f6 0%, #2563eb 100%)', color: 'white', borderRadius: '12px', fontSize: '14px', fontWeight: '600' },
    ...
    questionBoxActive: { borderColor: '#10b981', background: '#f0fdf4' },
      ...
      <div style={styles.content}>
        ...
        <div style={styles.header}>
          <div style={styles.headerTop}>
            <h1 style={styles.title}>📝 Profile {quizData.profile} Quiz</h1>
            <div style={styles.profileBadge}>
              Attempt {quizData.attemptsToday} of 2 Today
            </div>
          </div>
          <p style={styles.subtitle}>
            Score 70% or higher to advance to Profile {quizData.profile + 1}! Good luck! 🍀
          </p>
        ...
        <div style={styles.allQuestionsCard}>
          ...
          >
            {submitting ? '⏳ Submitting...' : `🚀 Submit Quiz (${answeredCount}/15 answered)`}
          </button>
        </div>
      </div>
    </div>
