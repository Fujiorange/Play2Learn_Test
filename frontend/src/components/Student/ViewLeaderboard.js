...
export default function ViewLeaderboard() {
  ...
  useEffect(() => {
    const loadLeaderboard = async () => {
      ...
      const mockData = [
        { rank: 1, name: 'Sarah Williams', points: 2850, level: 24, achievements: 48 },
        { rank: 2, name: 'Michael Chen', points: 2650, level: 22, achievements: 42 },
        { rank: 3, name: 'Emma Johnson', points: 2420, level: 21, achievements: 39 },
        { rank: 4, name: user.name || 'John Lim', points: 1250, level: 12, achievements: 24, isCurrentUser: true },
        { rank: 5, name: 'David Brown', points: 1180, level: 11, achievements: 22 },
      ];
            ...
            <tbody>
              {leaderboard.map(player => (
                <tr key={player.rank} style={player.isCurrentUser ? styles.currentUserRow : {}}>
                  ...
                  <td style={styles.td}><strong style={{ color: '#10b981' }}>{player.points.toLocaleString()}</strong></td>
                  <td style={styles.td}>Level {player.level}</td>
                  <td style={styles.td}>🏆 {player.achievements}</td>
                </tr>
              ))}
            </tbody>
          </table>
