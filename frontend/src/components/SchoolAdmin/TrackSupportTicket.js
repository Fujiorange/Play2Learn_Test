// TrackSupportTicket.js - School Admin version with reply display
...
export default function TrackSupportTicket() {
  ...
  useEffect(() => {
    const loadTickets = async () => {
      ...
      try {
        const result = await schoolAdminService.getMyTickets();
        ...
        <div style={styles.header}>
          <div style={styles.headerTop}>
            ...
            <div style={styles.buttonGroup}>
              <button style={{...styles.button, ...styles.backButton}} onClick={() => navigate('/school-admin')}>← Back</button>
              <button style={{...styles.button, ...styles.createButton}} onClick={() => navigate('/school-admin/support/create')}>+ New Ticket</button>
            </div>
          </div>
        ...
        {filteredTickets.length > 0 ? (
          <div style={styles.ticketGrid}>
            {filteredTickets.map(ticket => (
              <div key={ticket.id || ticket.ticketId} style={styles.ticketCard}>
                <div 
                  style={styles.ticketCardHeader}
                  onClick={() => toggleTicketExpand(ticket.id || ticket.ticketId)}
                >
                  <div style={styles.ticketHeader}>
                    <div style={styles.ticketIdRow}>
                      <span style={styles.ticketId}>#{(ticket.id || ticket.ticketId)?.toString().slice(-6).toUpperCase()}</span>
                      {ticket.hasReply && (
                  ...
                  <div style={styles.ticketInfo}>
                    ...
                    <span style={{ fontSize: '13px', color: '#6b7280' }}>📅 Created: {formatDate(ticket.created_at)}</span>
                    <span style={{ fontSize: '13px', color: '#6b7280' }}>🔄 Updated: {formatDate(ticket.updated_at)}</span>
                  </div>
                  <div style={styles.clickToExpand}>
                    {expandedTicket === (ticket.id || ticket.ticketId) ? '▲ Click to collapse' : '▼ Click to view details'}
                  </div>
                </div>

                {expandedTicket === (ticket.id || ticket.ticketId) && (
                  <div style={styles.expandedContent}>
                    ...
                    {ticket.hasReply ? (
                      ...
                      <div style={styles.noReply}>
                        ⏳ Awaiting P2L Admin response. We'll get back to you soon!
                      </div>
                    )}
                  </div>
                )}
          ...
          <div style={styles.emptyState}>
            ...
            <button
              onClick={() => navigate('/school-admin/support/create')}
              style={{
                marginTop: '20px',
                padding: '12px 24px',
